...
class TamilDhoolProvider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    try {
        if (!data.trim().startsWith("[")) {
            println("Invalid JSON input in loadLinks(): $data")
            return false
        }

        val link = parseJson<ArrayList<TamilDhoolLinks>>(data)

        val thiraione   = link.filter { it.sourceLink.contains("thirai", true) }
        val dailymotion = link.filter { it.sourceLink.contains("dailymotion", true) }
        val youtube     = link.filter { it.sourceLink.contains("youtube", true) }

        safeApiCall {
            if (thiraione.isNotEmpty()) {
                callback.invoke(
                    ExtractorLink(
                        thiraione[0].sourceName,
                        thiraione[0].sourceName,
                        thiraione[0].sourceLink.replace("/p/", "/v/") + ".m3u8",
                        thiraione[0].sourceLink,
            ...
            if (dailymotion.isNotEmpty()) {
                loadExtractor(dailymotion[0].sourceLink, subtitleCallback, callback)
            }
            if (youtube.isNotEmpty()) {
                loadExtractor(youtube[0].sourceLink, subtitleCallback, callback)
            }
        }

        return true
    } catch (e: Exception) {
        println("Failed to parse loadLinks JSON: ${e.message}")
        return false
    }
}
}
